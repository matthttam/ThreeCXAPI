...
class Warnings(TcxStrEnum):
    "WARNINGS.CONTACTS_SPECIFY_NAME_SURNAME_COMPANY" = auto()
    "WARNINGS.CONTACTS_SPECIFY_PHONE_NUMBER" = auto()
    "WARNINGS.LENGTH_NOT_MORE_50_CHARS" = auto()
    "WARNINGS.LENGTH_NOT_MORE_255_CHARS" = auto()
    "WARNINGS.XAPI.LENGTH_NOT_MORE_2048_CHARS" = auto()
    "WARNINGS.XAPI.INVALID_HEX_CHARACTER" = auto()
    "WARNINGS.NO_MORE_NUMBERS_AVAILABLE" = auto()
    "WARNINGS.ERP_SERVER_ERROR" = auto()
    "WARNINGS.LICENSE_NOT_FOUND" = auto()
    "WARNINGS.LIMIT_REACHED" = auto()
    "WARNINGS.XAPI.INVALID" = auto()
    "WARNINGS.XAPI.INVALID_PIN_NUMBER" = auto()
    "WARNINGS.XAPI.NOT_SUPPORTED" = auto()
    "WARNINGS.XAPI.USER_ROLE_DOWNGRADE" = auto()
    "WARNINGS.GROUP_CANNOT_BE_DELETED" = auto()
    "WARNINGS.CANNOT_BE_DELETED" = auto()
    "WARNINGS.GROUP_WITH_MEMBERS_CANNOT_BE_DELETED" = auto()
    "WARNINGS.XAPI.OTHER_USER_ROLE_DOWNGRADE" = auto()
    "WARNINGS.XAPI.INVALID_LICENSE_TYPE" = auto()
    "WARNINGS.XAPI.INVALID_PASSWORD" = auto()
    "WARNINGS.XAPI.NOT_FOUND" = auto()
    "WARNINGS.XAPI.FILE_NOT_FOUND" = auto()
    "WARNINGS.XAPI.FILE_NOT_ACCESSIBLE" = auto()
    "WARNINGS.XAPI.REQUIRED" = auto()
    "WARNINGS.XAPI.CAN_NOT_BE_EMPTY_STRING" = auto()
    "WARNINGS.XAPI.DUPLICATE" = auto()
    "WARNINGS.XAPI.ALREADY_IN_USE" = auto()
    "WARNINGS.XAPI.PLAYLIST_IN_USE" = auto()
    "WARNINGS.XAPI.OUT_OF_THE_RANGE" = auto()
    "WARNINGS.XAPI.TOO_MANY_PHONES" = auto()
    "WARNINGS.XAPI.TOO_MANY_SBC" = auto()
    "WARNINGS.XAPI.TOO_MANY_PROMPTS" = auto()
    "WARNINGS.XAPI.OUTBOUND_RULES_LIMIT_REACHED" = auto()
    "WARNINGS.XAPI.FORBIDDEN_CHANGE" = auto()
    "WARNINGS.FAX_SERVER_CANNOT_BE_DELETED" = auto()
    "WARNINGS.OPERATOR_CANNOT_BE_DELETED" = auto()
    "WARNINGS.USER_EXTENSION_CANNOT_BE_DELETED" = auto()
    "WARNINGS.XAPI.NUMBER_IGNORED" = auto()
    "WARNINGS.XAPI.INVALID_TIMEZONE" = auto()
    "WARNINGS.XAPI.INVALID_PATH" = auto()
    "WARNINGS.XAPI.PATH_SHOULD_NOT_CONTAIN_SPACES" = auto()
    "WARNINGS.XAPI.INVALID_CREDENTIALS" = auto()
    "WARNINGS.XAPI.CANNOT_CONNECT_FTP" = auto()
    "WARNINGS.XAPI.CANNOT_CONNECT_SMB" = auto()
    "WARNINGS.XAPI.CANNOT_CONNECT_SFTP" = auto()
    "WARNINGS.XAPI.CANNOT_CONNECT_GOOGLE_BUCKET" = auto()
    "WARNINGS.XAPI.PLAYLIST_NO_SOURCE" = auto()
    "WARNINGS.XAPI.NO_USERS_IN_TEAMS" = auto()
    "WARNINGS.XAPI.FILE_FORMAT_IS_INCORRECT" = auto()
    "WARNINGS.XAPI.INVALID_FILE_NAME" = auto()
    "WARNINGS.CSV_INVALID_FILE_FORMAT" = auto()
    "WARNINGS.CSV_LINE_CORRUPTED" = auto()
    "WARNINGS.WRONG_CSV_FILE_REQUIRED_COLUMNS_NOT_FOUND" = auto()
    "WARNINGS.CSV_IMPORT_LIMIT_REACHED" = auto()
    "WARNINGS.WRONG_CSV_FILE_REQUIRED_HEADER_NOT_FOUND" = auto()
    "WARNINGS.XAPI.FILE_IS_TOO_LARGE" = auto()
    "WARNINGS.XAPI.SBC_CERT_FQDN_MISMATCH" = auto()
    "WARNINGS.XAPI.SBC_CERT_EXPIRED" = auto()
    "WARNINGS.XAPI.SBC_KEY_CERT_MISMATCH" = auto()
    "WARNINGS.XAPI.NON_EXISTENT_EXT_NUMBER" = auto()
    "WARNINGS.XAPI.MCM_MODE_REQUIRED" = auto()
    "WARNINGS.INTERNATIONALPREFIX_IS_MISSING" = auto()
    "WARNINGS.TIMEZONEID_IS_MISSING" = auto()
    "WARNINGS.XAPI.CHAT_LOG_IS_DISABLED" = auto()
    "WARNINGS.WAKEUP_IVR_EXISTS" = auto()
    "WARNINGS.RING_GROUP_ENABLE_PAGING" = auto()
    "WARNINGS.XAPI.CREATE_1_SIP_TRUCK_EMERGENCY" = auto()
    "WARNINGS.DELETING_ALREADY_IN_PROGRESS" = auto()
    "WARNINGS.INVALID_IP_MASK" = auto()
    "WARNINGS.TOO_MANY_BACKUPS" = auto()
    "WARNINGS.BACKUP_LOCATION_CONFIG_ERROR" = auto()
    "WARNINGS.BACKUP_NOT_FOUND_OR_INVALID" = auto()
    "WARNINGS.INVALID_CALL_FLOW_FILE" = auto()
    "WARNINGS.ALREADY_EXPIRED" = auto()
    "WARNINGS.CALL_FLOW_MUST_BE_ALPHANUMERIC" = auto()
    "WARNINGS.EXTRACTING_OUTSIDE_THE_DESTINATION_DIRECTORY" = auto()
    "WARNINGS.INVALID_EXTENSION_NUMBER_LENGTH" = auto()
    "WARNINGS.DN_NUMBER_CANNOT_BE_USED" = auto()
    "WARNINGS.WIRESHARK_NOT_FOUND" = auto()
    "WARNINGS.CAPTURE_LOCALHOST_NOT_ALLOWED" = auto()
    "WARNINGS.CAPTURE_ONGOING" = auto()
    "WARNINGS.CANNOT_DELETE_TRUNKS_BINDED_ERMERGENCY_NUMBER" = auto()
    "WARNINGS.BLACKLIST_NUMBER_LIMIT_EXCEEDED" = auto()
    "WARNINGS.DOUBLE_QUOTES_NOT_ALLOWED" = auto()
    "WARNINGS.MCU_REQUEST_ALREADY_IN_PROGRESS" = auto()
    "WARNINGS.MCU_LIMIT_REACHED" = auto()
    "WARNINGS.MCU_WEBMEETING_BRIDGE_NOT_FOUND" = auto()
    "WARNINGS.MCU_REQUEST_NOT_FOUND" = auto()
    "WARNINGS.MCU_REQUEST_TIMEOUT" = auto()
    "WARNINGS.SUPPORTED_MEDIA_FORMAT_WAV" = auto()
    "WARNINGS.NO_SECRET_DEFINED" = auto()
    "WARNINGS.INVALID_SECURITY_CODE" = auto()
    "WARNINGS.UNABLE_REACH_UPDATES_SERVER" = auto()
    "WARNINGS.ERROR_DOWNLOADING_FROM_UPDATES_SERVER" = auto()